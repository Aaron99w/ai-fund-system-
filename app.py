...
def get_news_sentiment():
    try:
        ...
        headlines = list(dict.fromkeys(all_news))[:20]
        ...
        if pos_score + neg_score == 0:
            ...
            sentiment, emoji = "中性", "😐"
        return {"sentiment": sentiment, "emoji": emoji, "headlines": headlines, "source": "东方财富（头条+快讯）"}
    except:
        return {"sentiment": "中性", "emoji": "😐", "headlines": ["市场震荡整理，等待方向选择", "北向资金小幅流入", "政策预期升温", "板块轮动加快", "成交量萎缩"], "source": "模拟数据"}
...
def generate_signals(df):
    ...
    buy_signal = ((ma5 > ma10) & (ma10 > ma20) & (df['close'] > ma5) & (rsi > 40) & (rsi < 70) & (macd > signal_line) & (hist > 0))
    sell_signal = ((ma5 < ma10) & (ma10 < ma20) & (df['close'] < ma5) & (rsi > 60) & (rsi < 85) & (macd < signal_line) & (hist < 0))
